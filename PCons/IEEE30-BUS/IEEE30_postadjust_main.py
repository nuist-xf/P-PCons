...
rcParams['font.sans-serif'] = ['SimHei', 'Microsoft YaHei']  # 多备选字体
rcParams['axes.unicode_minus'] = False
rcParams['font.size'] = 16

from adjust_10000_OnOff import adjust_10000_unit_operations
from cons_adjust_function import enforce_ramping_constraints, multi_segment_clamp, enforce_status_power_consistency, allocate_power_deficit, allocate_power_surplus, enforce_ptdf_constraints

# 参数配置
DAYS = 30
PERIODS_PER_DAY = 288  # 5分钟间隔 (24h*60/5=288)
NUM_UNITS = 6
...
def load_data(model):
    ...
    full_data = pd.read_csv(os.path.join(input_dir, 'test_predictions.csv'))  # 文件名标准化
    ...
    # 读取约束文件
    constraints = pd.read_csv('data/IEEE30_constraints.csv', encoding='gb18030')
    assert len(constraints) == NUM_UNITS, "约束文件机组数不匹配"

    # 转换时间单位为时段数（假设约束单位为小时）
...
def load_ptdf_data():
    """独立加载PTDF相关数据（与原有load_data函数解耦）"""
    # ---- 加载PTDF矩阵 ----
    ptdf = pd.read_csv('data/ptdf_30.csv', header=None).values

    # ---- 获取机组/负荷节点索引 ----
    case30_data = case30()
    gen_bus_indices = case30_data["gen"][:, 0].astype(int) - 1  # 转换为0-based索引
    load_bus_indices = np.where(case30_data["bus"][:, 2] > 0)[0]  # PQ节点作为负荷节点
    ...
    new_node_data = node_data.reshape((365, 288, 20))
    test_indices = [198, 89, 65, 137, 34, 148, 160, 77, 73, 20, 98, 325, 280, 188, 63,
                    114, 157, 141, 100, 124, 223, 29, 252, 11, 87, 279, 115, 166, 31, 168]
    load_profile = new_node_data[test_indices, :, :]  # 仅提取负荷节点

    # ---- 加载线路容量 ----
    line_limits = case30_data["branch"][:, 5].copy().astype(float)
    # print(f"line_limits[9]",line_limits[9])
    line_limits[9] = 35  # 修正特定线路容量

    # 维度验证
    assert gen_ptdf.shape[1] == 6, "机组PTDF列数应为6"
...
def compare_results(before_clamped, before_status, after_clamped, after_status):
    """结果对比分析（示例）"""
    print("\n=== 修正前后对比 ===")
    print(f"启停次数变化：{np.sum(before_status != after_status)} 次")
    print(f"平均功率差异：{np.mean(after_clamped - before_clamped):.2f} MW")


def process_before_adjustment(predictions_3d, demand_data, constraints, output_dir):
    """修正前处理逻辑"""
    clamped_3d = predictions_3d.copy()
...
def main():
    global model
    # 在main函数中创建阶段目录
    output_dir_before = os.path.join('out', model, 'before_adjustment')
    output_dir_after = os.path.join('out', model, 'after_adjustment')
    os.makedirs(output_dir_before, exist_ok=True)
    os.makedirs(output_dir_after, exist_ok=True)


    # 数据加载
    predictions_3d, demand_data, constraints = load_data(model)
    gen_ptdf, load_ptdf, load_profile, line_limits = load_ptdf_data()

    # 修正前阶段
    print("\n=== 修正前处理 ===")
    output_dir_before = os.path.join('out', model, 'before_adjustment')
    clamped_before, status_before = process_before_adjustment(
        predictions_3d, demand_data, constraints, output_dir_before
    )

    # 修正后阶段
    print("\n=== 修正后处理 ===")
    output_dir_after = os.path.join('out', model, 'after_adjustment')
    clamped_after, status_after = process_after_adjustment(
        predictions_3d, demand_data, constraints,
        gen_ptdf, load_ptdf, load_profile, line_limits,
        output_dir_after
    )

    # 对比分析（可选）
    # compare_results(clamped_before, status_before, clamped_after, status_after)
